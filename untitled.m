...
hold on; [B,L,N] = bwboundaries(binaryImage,'noholes');
    for k=1:length(B),
    	boundary = B{k};
    	plot(boundary(:,2), boundary(:,1),'.r')
    end

xy = [boundary(:,2), boundary(:,1)]; % 2 1-D vectors.
...
path = xy';
pathAll = [0.25*path; zeros(1,numcols(path))];

path1 = [0.5*path; zeros(1,numcols(path))];
path2 = [0.5*path; zeros(1,numcols(path))];

path3 = [zeros(1,numcols(path)); 0.5*path];
path4 = [zeros(1,numcols(path)); 0.5*path];
